...
\begin{tikzpicture}[
  ...
  sliver_ep/.style={draw=pTERRA, fill=pTERRA!10, line width=0.7pt, minimum width=5mm,
                    inner sep=0pt, rounded corners=1.5pt, dash pattern=on 2pt off 1.5pt},
  lbl/.style={text=pSUBINK, font=\sffamily\scriptsize},
  tinylbl/.style={text=pSUBINK, font=\sffamily\scriptsize},
  callout/.style={text=pSUBINK, font=\sffamily\scriptsize, align=left},
  panel_lbl/.style={text=pINK, font=\sffamily\small\bfseries},
...
\node[episodic, minimum width=26mm] (netlist) at (-12mm,\rowtop) {Netlist\\\scriptsize(Verilog)};
\node[episodic, minimum width=26mm, right=11mm of netlist] (graph) {Netlist graph\\\scriptsize H-block nodes\\\scriptsize + fabric node};
\node[episodic, minimum width=26mm, right=11mm of graph] (pad) {Pad to\\fixed capacity\\\scriptsize max.\ nodes/edges};
...
\node[text=pINK, font=\sffamily\small\bfseries, align=center] (occ_lbl) at ($(occ_pos)+(0,-20mm)$)
      {Occupancy grid\\[-0.2ex]\normalfont\scriptsize current placement state, per step\\[-0.2ex]\normalfont\scriptsize once per step};
\node[minimum height=3mm, below=0mm of occ_lbl] (panelA_pad) {};

% --- Zero-shot unseen netlist, below netlist ---
\node[zsdata, minimum width=26mm] (unseen) at (netlist |- {(0,-32mm)}) {Unseen netlist\\\scriptsize never trained on};
\draw[zsarrow] (unseen.north) -- (netlist.south);
\node[lbl, below=2mm of unseen, text width=30mm, align=center] (unseen_lbl)
      {same checkpoint,\\[-0.5ex]single rollout $\to$ \S 5.3};

% --- valid(w,h): per-episode data, aligned under graph column ---
\node[episodic, text width=26mm, align=center] (validwh) at (graph |- {(0,-90mm)})
      {valid $(w,h)$\\\scriptsize norm.\ footprint};
...
\node[text=pINK, font=\sffamily\small\bfseries] (gcn_lbl) at ($(gcn_box.north)+(0,5mm)$) {GCN encoder};
\node[text=pSUBINK, font=\sffamily\scriptsize, align=center] (gcn_sub) at ($(gcn_box.south)+(0,-5mm)$)
      {2$\times$ GCNConv (64)\\[-0.5ex]$\to$ node embeddings};
...
\node[text=pINK, font=\sffamily\small\bfseries] (cnn_lbl) at ($(cnn_box.north)+(0,5mm)$) {CNN encoder};
\node[text=pSUBINK, font=\sffamily\scriptsize, align=center] (cnn_sub) at ($(cnn_box.south)+(0,-5mm)$)
      {4-channel grid\\[-0.5ex]AdaptiveAvgPool $\to$ Linear};
\begin{scope}[shift={($(cnn_box.center)+(-0.35,-0.05)$)}]
  \pic (cnn1) at (-1.0,0)  {cuboid={width=0.6, height=1.3, depth=1.2, fill=pTEAL!18, draw=pTEAL}};
...
\node[lbl, align=right, anchor=south east] (pool_lbl)
      at ($(emb_pool.west) + (-2mm, 0)$) {mean-pooled\\[-0.5ex]whole-netlist\\[-0.5ex]\scriptsize 64-d};
...
\node[lbl, align=right, anchor=south east] (curr_lbl)
      at ($(emb_curr.west) + (-2mm, 0)$) {current\\[-0.5ex]H-block\\[-0.5ex]\scriptsize 64-d};

% --- CNN output: clean horizontal into emb_cnn (same row) ---
\draw[arr] (cnn_box.east) -- node[midway, above=0.8mm, lbl, align=center]
      {CNN output\\[-0.5ex]\scriptsize 64-d} (emb_cnn.west);

% --- valid(w,h) -> emb_wh : per-episode footprint crosses into the concat ---
\draw[arr] (validwh.east) -- node[pos=0.85, above=0.8mm, lbl] {\scriptsize 2-d} (emb_wh.west);
...
\node[text=pINK, font=\sffamily\small\bfseries, align=center] (concat_lbl) at ($(concat.north)+(0,7mm)$)
      {Concatenated\\[-0.5ex]\normalfont\scriptsize (64+64+64+2) = \textbf{194-d}};
...
\node[text=pINK, font=\sffamily\small\bfseries, align=center] (fuse_lbl) at ($(fuse.north)+(0,7mm)$)
      {Fuse $\to$ \textbf{128-d}};
\node[text=pSUBINK, font=\sffamily\scriptsize, align=center] (fuse_sub) at ($(fuse.south)+(0,-3.5mm)$)
      {Linear(194 $\to$ 128)\\[-0.5ex]+ ReLU};
...
% PPO AGENT
% =========================================================
\node[fixed, minimum height=11mm, minimum width=24mm] (actor)  at ($(fuse.east)+(30mm,8mm)$)  {Actor\\\scriptsize (masked action)};
\node[fixed, minimum height=11mm, minimum width=24mm] (critic) at ($(fuse.east)+(30mm,-12mm)$) {Critic\\\scriptsize (value)};
\node[draw=pTEAL, dashed, rounded corners=5pt, fit=(actor)(critic), inner sep=7pt] (ppo_sys) {};
\node[text=pINK, font=\sffamily\small\bfseries] (ppo_lbl) at ($(ppo_sys.north)+(0,5mm)$) {PPO Agent};
\node[lbl, align=center, text width=32mm] (mask_lbl) at ($(ppo_sys.south)+(0,-5mm)$)
      {mask: valid coords, active benchmark\\loop: repeats per H-block};
...
% =========================================================
\node[ourcode, minimum width=30mm, right=30mm of actor] (bake) {Render layout\\\scriptsize Jinja2 $\to$ arch.\ XML};
\node[oracle, minimum width=30mm, right=12mm of bake] (vtr)
      {VTR oracle\\\scriptsize synth/pack/place/route\\\scriptsize one call per episode};
\node[oracle, minimum width=30mm, below=12mm of vtr] (reward) {ADP $\to$ reward\\\scriptsize Eq. 3};
\node[ourcode, minimum width=30mm, below=12mm of bake] (baseline)
      {Baseline metrics\\\scriptsize Area$_0$,Delay$_0$,Power$_0$};

\draw[arr] (actor.east) -- node[midway, above=1mm, lbl, align=center] {placed coords\\[-0.5ex]\scriptsize per H-block} (bake.west);
\draw[arr] (bake) -- (vtr);
\draw[arr] (vtr) -- (reward);
...
\draw[loopback] (panelC.south) -- (panelC.south |- loopback_y)
    -| node[pos=0.20, above=1.5mm, lbl, text width=46mm, align=center]
       {PPO gradient (training only)\\[-0.5ex]\scriptsize updates GCN, CNN, Fuse, Actor, Critic}
       (panelB.south);

% =========================================================
% LEGEND
% =========================================================
\node[legendkey, fill=pTERRA!12, draw=pTERRA] (lg1) at ($(current bounding box.south)+(-118mm,-16mm)$) {};
...
\caption{System overview: a netlist becomes a fixed-schema graph (Panel A),
encoded jointly with the occupancy grid by a fixed-weight GCN+CNN policy that
emits aspect ratio and block coordinates under action masking (Panel B); the
policy decodes one H-block per step, updating the occupancy grid, until the
full layout is placed. Our own code then renders that layout into an
architecture XML, which the real, external VTR flow scores once per episode
(Panel C), producing the reward (computed against the traditional baseline)
that updates the policy (training only). The same trained weights accept any
netlist that fits within the fixed-capacity superset, including ones never
seen during training (dashed path, bottom-left): this is the zero-shot path
