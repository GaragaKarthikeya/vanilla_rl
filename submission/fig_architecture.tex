...
\tikzset{
    cuboid/.pic={
        ...
        \draw[fill=\pgfkeysvalueof{/cuboid/fill}, draw=\pgfkeysvalueof{/cuboid/draw}, line width=1.2pt, join=round]
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners=3pt, minimum height=12mm, minimum width=28mm,
              align=center, line width=1.2pt, text=pINK, inner sep=6pt},
  ...
  panel/.style={draw=pGRAY, dashed, rounded corners=6pt, inner sep=14pt, line width=1.0pt},
  % tensor slivers: dashed border + lighter fill, to distinguish from solid module boxes
  sliver/.style={draw=pTEAL, fill=pTEAL!10, line width=1.2pt, minimum width=5mm,
                 inner sep=0pt, rounded corners=1.5pt, dash pattern=on 2pt off 1.5pt},
  sliver_ep/.style={draw=pTERRA, fill=pTERRA!10, line width=1.2pt, minimum width=5mm,
                    inner sep=0pt, rounded corners=1.5pt, dash pattern=on 2pt off 1.5pt},
  lbl/.style={text=pSUBINK, font=\sffamily\large},
  tinylbl/.style={text=pSUBINK, font=\sffamily\large},
  callout/.style={text=pSUBINK, font=\sffamily\Large, align=left},
  panel_lbl/.style={text=pINK, font=\sffamily\Large\bfseries},
  arr/.style={-{Triangle[length=2.5mm,width=2mm]}, line width=1.5pt, draw=pINK, rounded corners=3pt},
  loopback/.style={arr, draw=pTERRA},
  leader/.style={-{Triangle[length=2mm,width=1.5mm]}, line width=1.5pt, draw=pSUBINK, dash pattern=on 1.5pt off 1.5pt},
  zsarrow/.style={arr, draw=pTERRA, dash pattern=on 3pt off 2pt},
  legendkey/.style={draw, rounded corners=1.5pt, minimum width=5mm, minimum height=3mm, line width=1.0pt},
  legendkey_tensor/.style={draw, rounded corners=1.5pt, minimum width=5mm, minimum height=3mm, line width=1.0pt, dash pattern=on 2pt off 1.5pt},
]
...
\node[episodic, minimum width=26mm] (netlist) at (-30mm,\rowtop) {Netlist\\[-0.3ex]\large(Verilog)};
\node[episodic, minimum width=26mm, right=11mm of netlist] (graph) {Netlist graph\\[-0.3ex]\large H-block nodes+fabric node};
\node[episodic, minimum width=26mm, right=11mm of graph] (pad) {Pad to\\[-0.3ex]fixed capacity\\[-0.3ex]\large max.\ nodes/edges};
...
\node[text=pINK, font=\sffamily\Large\bfseries, align=center] (occ_lbl) at ($(occ_pos)+(0,-20mm)$)
      {Occupancy grid\\[-0.3ex]\normalfont\large current placement state, per step\\[-0.3ex]\normalfont\large once per step};
...
\node[episodic, text width=26mm, align=center] (validwh) at (graph |- {(0,-90mm)})
      {valid $(w,h)$\\[-0.3ex]\large norm.\ footprint};
...
% =========================================================
\def\enccol{134mm}   % x-position of the encoder column
...
\node[text=pSUBINK, font=\sffamily\Large, align=center] (gcn_sub) at ($(gcn_box.south)+(0,-5mm)$)
...
\node[text=pSUBINK, font=\sffamily\Large, align=center] (cnn_sub) at ($(cnn_box.south)+(0,-5mm)$)
...
\begin{scope}[shift={($(cnn_box.center)+(-0.35,-0.05)$)}]
  ...
  \foreach \mid in {m1,m2,m3} {
    ...
    \draw[pTEAL, line width=1.0pt] (\mid) circle (1.0pt);
  }
\end{scope}
...
\draw[pINK, line width=1.5pt] (gcn_box.east) -- (gcn_split);
\draw[pINK, line width=1.5pt, rounded corners=3pt] (gcn_split) |- ($(emb_pool.west)+(-12mm,0)$) -- (emb_pool.west);
\draw[arr] ($(emb_pool.west)+(-2pt,0)$) -- (emb_pool.west);
\node[lbl, align=left, anchor=south west] (pool_lbl)
      at ($(emb_pool.north west) + (0, 0.5mm)$) {mean-pooled\\[-0.3ex]whole-netlist\\[-0.3ex]\large 64-d};

\draw[pINK, line width=1.5pt, rounded corners=3pt] (gcn_split) |- ($(emb_curr.west)+(-12mm,0)$) -- (emb_curr.west);
\draw[arr] ($(emb_curr.west)+(-2pt,0)$) -- (emb_curr.west);
\node[lbl, align=left, anchor=north west] (curr_lbl)
      at ($(emb_curr.south west) + (0, -0.5mm)$) {current\\[-0.3ex]H-block\\[-0.3ex]\large 64-d};

% --- CNN output: clean horizontal into emb_cnn (same row) ---
\draw[arr] (cnn_box.east) -- (emb_cnn.west);
\node[lbl, align=left, anchor=north west] (cnn_lbl)
      at ($(emb_cnn.south west) + (0, -0.5mm)$) {CNN output\\[-0.3ex]\large 64-d};

% --- valid(w,h) -> emb_wh : per-episode footprint crosses into the concat ---
\draw[arr] (validwh.east) -- (emb_wh.west);
\node[lbl, align=left, anchor=north west] (wh_lbl)
      at ($(emb_wh.south west) + (0, -0.5mm)$) {\large 2-d};
...
\node[text=pINK, font=\sffamily\Large\bfseries, align=center] (fuse_lbl) at ($(fuse.north)+(0,7mm)$)
      {Fuse \& Project\\[-0.3ex]\normalfont\large \textbf{128-d}};
\node[text=pSUBINK, font=\sffamily\Large, align=center] (fuse_sub) at ($(fuse.south)+(0,-3.5mm)$)
      {Linear + ReLU};
...
\begin{scope}[on background layer]
  \foreach \i in {0.05,0.15,0.25,0.35,0.45,0.55,0.65,0.75,0.85,0.95}{
    \foreach \j in {0.15,0.4,0.6,0.85}{
      \draw[pTEAL, opacity=0.28, line width=1.5pt]
...
% PPO AGENT
% =========================================================
\node[fixed, minimum height=11mm, minimum width=24mm] (actor)  at ($(fuse.east)+(30mm,8mm)$)  {Actor\\[-0.3ex]\large (masked action)};
\node[fixed, minimum height=11mm, minimum width=24mm] (critic) at ($(fuse.east)+(30mm,-12mm)$) {Critic\\[-0.3ex]\large (value)};
...
\node[lbl, align=center, text width=38mm] (mask_lbl) at ($(ppo_sys.south)+(0,-10mm)$)
...
% =========================================================
\node[ourcode, minimum width=30mm, right=30mm of actor] (bake) {Render layout\\[-0.3ex]\large Jinja2 $\to$ arch.\ XML};
\node[oracle, minimum width=30mm, right=12mm of bake] (vtr)
      {VTR oracle\\[-0.3ex]\large synth/pack/place/route\\[-0.3ex]\large one call per episode};
\node[ourcode, minimum width=30mm, below=12mm of bake] (baseline)
      {Baseline metrics\\[-0.3ex]\large Area$_0$,Delay$_0$,Power$_0$};
\node[oracle, minimum width=30mm] (reward) at (vtr |- baseline) {ADP $\to$ reward\\[-0.3ex]\large Eq. 3};
...
\begin{scope}[on background layer]
\node[panel,
      fit=(netlist)(graph)(pad)(pad_tiny)(feat)(occ_anchor)(occ_lbl)(panelA_pad)(validwh)(validwh_pad),
...
\coordinate (loopback_y) at (0,-130mm);
\draw[loopback] (panelC.south) -- (panelC.south |- loopback_y)
    -| node[pos=0.20, above=1.5mm, lbl, text width=46mm, align=center]
       {PPO gradient (training only)\\[-0.3ex]\large updates GCN, CNN, Fuse, Actor, Critic}
       (panelB.south);

% =========================================================
% LEGEND
% =========================================================
\node[legendkey, fill=pTERRA!12, draw=pTERRA] (lg1) at ($(current bounding box.south)+(-141mm,-16mm)$) {};
...
\node[minimum width=5mm, minimum height=3mm, right=8mm of lg3t] (lg5) {\tikz{\draw[loopback] (0,0) -- (5mm,0);}};
\node[lbl, anchor=west, right=2.5mm of lg5] (lg5t) {training-only update};

\end{tikzpicture}%
\ifnum\figisstandalone=1\else
}
\caption{System overview of the RL-based fabric layout search. The left panel encodes the input netlist and grid. The center panel rolls out the placement sequentially, positioning one H-block per step under action masking. The right panel uses Jinja2 to render the final placement into a VTR architecture XML for verification and reward computation.}
\Description{Three-panel system diagram: the left panel shows the input
netlist encoded as a graph and the fabric grid; the center panel shows the
RL policy placing one heterogeneous block per step under action masking;
the right panel shows the completed placement rendered into a VTR
architecture XML and evaluated to produce area, delay, and power for the
reward.}
